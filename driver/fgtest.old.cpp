...
void dump_fd_flags(const char* name, int fd) {
    ...
    fprintf(stderr,
        "[%s] fd=%d F_GETFL=0x%x F_GETFD=0x%x (CLOEXEC=%s)\n",
        name,
        fd,
        status_flags,
        fd_flags,
...
static bool __solve_expr(z3::expr &e) {
  ...
  opt_solver.set("timeout", 10000U); // follow timeout of symcc
  opt_solver.add(e);
  ...
  if (res == z3::sat) {
    ...
    if (res == z3::sat) {
      ...
      AOUT("Unoptimistic case rejected!\n");
    #if OPTIMISTIC
...
static void __solve_cond(dfsan_label label, u8 r, bool add_nested, u64 addr) {
  ...
  try {
    ...
    __z3_solver.reset();
    AOUT("About to maybe add expression\n");
...
int main(int argc, char* const argv[]) {
  ...
  // }
  printf("DEBUG: This is a stdout test.\n");
  ...
  dump_fd_flags("pipe read end", pipefds[0]);
  dump_fd_flags("pipe write end", pipefds[1]);
  ...
  flags &= ~FD_CLOEXEC;
  if (fcntl(pipefds[1], F_SETFD, flags) == -1) {
  ...
  if (pid == 0) {
    ...
    fprintf(stderr, "DEBUG: Wrote to TAINT_OPTIONS: %s\n", options);
    char* args[argc];
    for (int i = 0; i < argc-1; i++) {
      args[i] = argv[i+1];
      fprintf(stderr, "DEBUG: Wrote argument %d: %s\n", i, args[i]);
    }
    args[argc-1] = NULL;
    execv(program, args);
    perror("FATAL: execv failed");
    // CRITICAL: Exit with a non-zero status to signal failure.
    _exit(127); // Use _exit to avoid flushing buffers
  }
  ...
  fprintf(stderr, "DEBUG: SYMCC_AFL_COVERAGE_MAP is %s\n", bitmap);
  ...
  // uint64_t last_pc = 0;
  fprintf(stderr, "DEBUG: pipefds[0] is %d\n", pipefds[0]);
  dump_fd_flags("pipe read end", pipefds[0]);
  dump_fd_flags("pipe write end", pipefds[1]);
  ssize_t readstatus;
  while (readstatus = read(pipefds[0], &msg, sizeof(msg)) > 0) {
    AOUT("DEBUG: This is a test message in the while loop!\n");
    // solve constraints
    switch (msg.msg_type) {
      fprintf(stderr, "DEBUG: msg_type is %d\n", msg.msg_type);
      case cond_type:
        ...
        else AOUT("DEBUG: Branch defined as not interesting.\n");
        break;
      case gep_type:
  ...
  fprintf(stderr, "DEBUG: end of read loop, status variable returns %d\n", readstatus);
